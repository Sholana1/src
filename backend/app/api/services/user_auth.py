...
from backend.app.auth.schema import AccountStatus, UserCreateSchema
from backend.app.core.utils import (
    generate_password_hash, generate_username, generate_otp, verify_password, create_activation_token
)
...
class UserAuthService:
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.info(f"OTP sent to user {user.email} successfully")
                    return True, otp
            ...
            logger.error(f"Failed to generate and save OTP")
            user.otp=""
            await session.commit()
            await session.refresh(user)
            return False, ""
    ...
    async def increment_failed_login_attempts(self, user:User, session:AsyncSession) -> None:
        ...
        if user.failed_login_attempts >= settings.LOGIN_ATTEMPT:
            ...
            try:
                ...
                logger.error(f"Fail to send account lockout email to {user.email}:{e}")
    ...
    async def reset_password(self, token:str, new_password:str, session:AsyncSession)-> None:
        try:
            payload = jwt.decode(
                token,
                settings.JWT_SECRET_KEY,

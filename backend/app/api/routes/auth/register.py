    ...
    try:
        if await user_auth_service.check_user_email_exists(user_data.email, session):
            raise HTTPException(
                ...
                detail="User with this credentials already exits"
            )
        
        if await user_auth_service.check_user_id_no_exists(user_data.id_no, session):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="User with this credential already exits"
            )
            
        new_user = await user_auth_service.create_user(user_data, session)
        logger.info(f"New user {user_data.email} registered successfully, awaiting activation")
        return new_user

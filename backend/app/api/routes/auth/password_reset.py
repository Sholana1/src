...
async def request_password_request(reset_data:PasswordResetRequestSchema, session:AsyncSession = Depends(get_session))-> dict:
    try:
        ...
        return {
            "message": "If an account exits with this email, "
...
async def reset_password(token:str, reset_data:PasswordResetConfirmSchema, session:AsyncSession = Depends(get_session)):
    try:
        ...
        logger.error(f"Password resee failed {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "status": "error",

...
async def get_current_user(session:AsyncSession = Depends(get_session), access_token:str | None = Cookie(None, alias=settings.COOKIE_ACCESS_NAME)) -> User:
    if not access_token:
        raise HTTPException(

...
def verify_password(password:str, hash_password:str) -> bool:
    try:
        return _ph.verify(hash_password, password)
...
def create_jwt_token(id:uuid.UUID, type:str = settings.COOKIE_ACCESS_NAME) -> str:
    expire_minutes = (
        settings.JWT_ACCESS_TOKEN_EXPIRATION_MINUTES if settings.COOKIE_ACCESS_NAME else settings.JWT_REFRESH_TOKEN_EXPIRATION_DAYS
    )
    ...
    return jwt.encode(payload, settings.SIGNIN_KEY, algorithm=settings.JWT_ALGORITHM)
...
def set_auth_cookies(
    ...
    logged_in_cookie_settings["httponly"] = (
        False
    )
